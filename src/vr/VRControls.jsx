...
import { CameraController } from "@/world/CameraController.jsx";
import { ProductGSAPUtil }  from "@/world/ProductGSAPUtil.jsx";
...
export const RapierVRProxy = () => {
  // Set player speed based on environment
  const [moveSpeed, setMoveSpeed] = useState(0);
  const {environmentType} = useEnvironmentStore();
  const originRef = useRef();
  const offsetQuaternionRef = useRef(new THREE.Quaternion());
  const cameraWithOffsetQuaternionRef = useRef(new THREE.Quaternion());
  ...
  const rotationState = useXRInputSourceState('controller', 'right');

  useFrame(() => {
    if (!playerRef.current || isAnimating ) return;
    const gamepad = controllerState?.gamepad['xr-standard-thumbstick']
    const rotationPad = rotationState?.gamepad['xr-standard-thumbstick']
    ...
    if (crosshairVisible) {
      ...
      // Map joystick axes to directional magnitudes using sign with a small deadzone
      const DEADZONE = 0.08;
      const x = gamepad?.xAxis || 0;
      const y = gamepad?.yAxis || 0;
      const Rotationx = rotationPad?.xAxis || 0;

      const left = x < -DEADZONE ? -x : 0;
      const right = x > DEADZONE ? x : 0;
      const forward = y < -DEADZONE ? -y : 0; // typically -Y is forward
      const backward = y > DEADZONE ? y : 0;
      const leftRotation = Rotationx < -DEADZONE ? -Rotationx : 0;
      const rightRotation = Rotationx > DEADZONE ? Rotationx : 0;

      if (leftRotation !== 0 || rightRotation !== 0) {
        if (rightRotation) {
          originRef.current.rotation.y -= 0.01;
        } else if (leftRotation) {
          originRef.current.rotation.y += 0.01;
        }
        originRef.current.updateMatrixWorld(true);
      }
      ...
      offsetQuaternionRef.current.setFromAxisAngle(THREE.Object3D.DEFAULT_UP, originRef.current?.rotation.y || 0);
      cameraWithOffsetQuaternionRef.current.copy(camera.quaternion).multiply(offsetQuaternionRef.current);
  

      movementDirection
        .copy(combinedInput)
        .applyQuaternion(cameraWithOffsetQuaternionRef.current) 
        .normalize()
    ...
    originRef.current.position.set( x - cameraOffset.x, y - cameraOffset.y, z - cameraOffset.z );
  });
